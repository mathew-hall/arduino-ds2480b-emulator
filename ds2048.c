...
void ds2480_cmd(enum ds2480_mode *mode, uint8_t cmd){
    if(*mode == POWERON){
        uint8_t reset_byte = (uint8_t) ( CMD_COMM | FUNCTSEL_RESET | SPEEDSEL_STD );
        if(cmd == reset_byte) {
            output(0x00); //ack
            *mode = DATA; // Transition to data mode after reset
        } else {
            LOGF("Invalid command in POWERON mode: 0x%02X", cmd);
            return;
        }
    }
    switch(*mode) {
        case DATA:
            // Handle data mode command
            break;
        case CHECK:
            // Handle check mode command
            break;
        case COMMAND:
            // Handle command mode command
            break;
    }
}